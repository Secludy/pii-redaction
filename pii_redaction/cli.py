...
def main():
    ...
    def add_pii_handling_args(parser):
        ...
        mode_group.add_argument('--tag', action='store_const', dest='mode', const=PIIHandlingMode.TAG,
                              help='Keep PII content between XML tags (default)')
        mode_group.add_argument('--redact', action='store_const', dest='mode', const=PIIHandlingMode.REDACT,
                              help='Replace PII with just an empty tag')
        mode_group.add_argument('--replace', action='store_const', dest='mode', const=PIIHandlingMode.REPLACE,
    ...
    jsonl_parser = subparsers.add_parser('process-jsonl', help='Process a JSONL dataset to handle PII in message content')
    jsonl_parser.add_argument('input', help='Input JSONL file')
    jsonl_parser.add_argument('output', help='Output JSONL file')
    jsonl_parser.add_argument('--device', help='Device to use for processing (e.g., cuda, cpu)')
    add_pii_handling_args(jsonl_parser)
    
    # Process text files command
    text_parser = subparsers.add_parser('process-text', help='Process a text file with one document per line')
    ...
    text_parser.add_argument('--device', help='Device to use for processing (e.g., cuda, cpu)')
    ...
    if args.command == 'process-jsonl':
        ...
        print(f"{action} PII in {len(tagged_documents)} documents and saved to {args.output}")
    else:
        parser.print_help()
