...
class PIIType(Enum):
    ...
    CREDIT_CARD_INFO = "credit_card_info"  # A credit card number, expiration date, CCV, etc.
    ...
    DEMOGRAPHIC_GROUP = "demographic_group"  # Anything that identifies race or ethnicity
    GENDER = "gender"  # A gender identifier
    PERSONAL_ID = "personal_id"  # Any ID string like a national ID, subscriber number, etc.
    ...
    RELIGIOUS_AFFILIATION = "religious_affiliation"  # Anything that identifies religious affiliation
    ...
    while i < len(tagged_str):
        if tagged_str[i] == '<':
            ...
            if m:
                ...
                annotations.append((annotation_start, annotation_start + len(annotated_text), tag, annotated_text))
...
def apply_tags(original, tagged_strings, tags_to_include, mode=PIIHandlingMode.TAG, locale="en_US"):
...
class PIIRedactor:
    def __init__(self, device=None):
        ...
        self.focus_tags = [["person_name", "organization_name"]] + [None] * (len(self.model_paths) - 1)
    ...
    def _initialize_model(self, index):
        ...
        if self.models[index] is None:
            self.models[index] = AutoModelForCausalLM.from_pretrained(self.model_paths[index])
            self.tokenizers[index] = AutoTokenizer.from_pretrained(self.model_paths[index], padding_side="left")
...
def tag_pii_in_documents(documents, device=None, mode=PIIHandlingMode.TAG, locale="en_US"):
    """
...
def clean_dataset(input_filename, output_filename, device=None, mode=PIIHandlingMode.TAG, locale="en_US"):
    """
...
def process_and_write_batch(json_objs_batch, fout, redactor, 
                           mode=PIIHandlingMode.TAG, locale="en_US"):
    """
